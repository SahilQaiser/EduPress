...
<?php if ( bp_has_message_threads( bp_ajax_querystring( 'messages' ) ) ) : ?>
	...
	<form action="<?php echo bp_displayed_user_domain() . bp_get_messages_slug() . '/' . bp_current_action() ?>/bulk-manage/" method="post" id="messages-bulk-management">
			...
			<thead>
				<tr>
					<th scope="col" class="thread-checkbox bulk-select-all">
						...
						<label class="" for="select-all-messages"><?php
						/* translators: accessibility text */
					...
					<?php if ( bp_is_active( 'messages', 'star' ) ) : ?>
						<th scope="col" class="thread-star">
							<span class="message-action-star"><span class="icon"></span> <span class="screen-reader-text"><?php
							/* translators: accessibility text */
				...
				<?php while ( bp_message_threads() ) : bp_message_thread(); ?>
					...
					<tr id="m-<?php bp_message_thread_id(); ?>" class="<?php bp_message_css_class(); ?><?php if ( bp_message_thread_has_unread() ) : ?> unread<?php else: ?> read<?php endif; ?>">
						<td class="bulk-select-check">
							...
							<label for="bp-message-thread-<?php bp_message_thread_id(); ?>"></label>
						...
						<?php if ( 'sentbox' != bp_current_action() ) : ?>
							<td class="thread-from">
								<figure class="img-dynamic aspect-ratio avatar avatar-square-sm avatar-online">
									<div class="img-card">
						...
						<td class="thread-options">
							<?php if ( bp_message_thread_has_unread() ) : ?>
								<a class="read primary btn btn-xs" href="<?php bp_the_message_thread_mark_read_url( bp_displayed_user_id() );?>"><?php esc_html_e( 'Read', 'buddypress' ); ?></a>
							<?php else : ?>
								<a class="unread btn btn-xs" href="<?php bp_the_message_thread_mark_unread_url( bp_displayed_user_id() );?>"><?php esc_html_e( 'Unread', 'buddypress' ); ?></a>
							<?php endif; ?>
							<a class="delete btn btn-xs" href="<?php bp_message_thread_delete_link( bp_displayed_user_id() ); ?>"><i class="icon icon-trash"></i></a>
		...
		<div class="messages-options-nav">
			<?php svq_bp_messages_bulk_management_dropdown(); ?>
