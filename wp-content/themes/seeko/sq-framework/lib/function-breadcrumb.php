...
if ( ! function_exists( 'svq_breadcrumb' ) ) {
	/**
	 ...
	 * @return string|bool
	 */
	function svq_breadcrumb( $args = array() ) {
		$breadcrumb_filter = false;

		if ( function_exists( 'yoast_breadcrumb' ) ) {
			$yoast_breadcrumb = yoast_breadcrumb( '<div class="svq-breadcrumb svq-yoast py-4">', '</div>', 'false' );
		...
		$breadcrumb_filter = apply_filters( 'svq_breadcrumb_data', $breadcrumb_filter );
...
class Breadcrumb_Trail {
	...
	public function __construct( $args = array() ) {
		...
		$defaults = array(
			...
			/* Post taxonomy (examples follow) => 'post'  => 'post_tag' */
			'post_taxonomy' => array(),
	...
	public function trail() {
		...
		if ( ! empty( $this->items ) && is_array( $this->items ) ) {
			...
			$vocabulary = '';
			if ( true === $this->args['rich_snippet'] ) {
				$vocabulary = ' itemscope itemtype="http://schema.org/BreadcrumbList"';
			}

			/* Open the breadcrumb trail containers. */
			$breadcrumb = "\n\t\t" . '<' . tag_escape( $this->args['container'] ) . ' class="' . esc_attr( $this->args['container_class'] ) . '"' . $vocabulary . '>';
			...
			foreach ( $this->items as &$link ) {
				...
				if ( true === $this->args['rich_snippet'] ) {
					...
					$link       = str_replace( '<a ', '<a itemprop="item" ', $link, $count_replace );
					$micro_data = '';
					if ( $count_replace > 0 ) {
		...
		if ( true === $this->args['echo'] ) {
			$breadcrumb_escaped = $breadcrumb;

			// This variable has been safely built above with escaped dynamic components
			echo $breadcrumb_escaped; // WPCS: XSS OK.
		} else {
			return $breadcrumb;
		}
	}
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => __( 'Browse:', 'seeko' ),
			'home'                => __( 'Home', 'seeko' ),
			'error_404'           => __( '404 Not Found', 'seeko' ),
			'archives'            => __( 'Archives', 'seeko' ),
			/* Translators: %s is the search query. The HTML entities are opening and closing curly quotes. */
			'search'              => __( 'Search results for &#8220;%s&#8221;', 'seeko' ),
			/* Translators: %s is the page number. */
			'paged'               => __( 'Page %s', 'seeko' ),
			/* Translators: Minute archive title. %s is the minute time format. */
			'archive_minute'      => __( 'Minute %s', 'seeko' ),
			/* Translators: Weekly archive title. %s is the week date format. */
			'archive_week'        => __( 'Week %s', 'seeko' ),
	...
	public function do_post_parents( $post_id ) {
		...
		while ( $post_id ) {
			...
			$parents[] = '<a href="' . get_permalink( $post_id ) . '" title="' . esc_attr( $page_title ) . '">' . wp_kses_post( $page_title ) . '</a>';
	...
	public function do_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->args['labels']['archive_minute_hour'], get_the_time( _x( 'g:i a', 'minute and hour archives time format', 'seeko' ) ) );
		}
	}
	...
	public function do_minute_archive_items() {
		...
		/* Add the minute item. */
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->args['labels']['archive_minute'], get_the_time( _x( 'i', 'minute archives time format', 'seeko' ) ) );
		}
	}
	...
	public function do_hour_archive_items() {
		...
		/* Add the hour item. */
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->args['labels']['archive_hour'], get_the_time( _x( 'g a', 'hour archives time format', 'seeko' ) ) );
		}
	}
	...
	public function do_day_archive_items() {
		...
		$year  = sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format', 'seeko' ) ) );
		$month = sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'seeko' ) ) );
		$day   = sprintf( $this->args['labels']['archive_day'], get_the_time( _x( 'j', 'daily archives date format', 'seeko' ) ) );
	...
	public function do_week_archive_items() {
		...
		$year = sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format', 'seeko' ) ) );
		$week = sprintf( $this->args['labels']['archive_week'], get_the_time( _x( 'W', 'weekly archives date format', 'seeko' ) ) );
		...
		/* Add the week item. */
		if ( is_paged() ) {
			$this->items[] = get_archives_link( add_query_arg( array(
				'm' => get_the_time( 'Y' ),
				'w' => get_the_time( 'W' )
			), esc_url( home_url( '/' ) ) ), $week, false );
		} elseif ( true === $this->args['show_title'] ) {
			$this->items[] = $week;
		}
	}
	...
	public function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag ) {
					$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format', 'seeko' ) ) ) . '</a>';
				} /* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag ) {
					$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'seeko' ) ) ) . '</a>';
				} /* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag ) {
					$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_day'], get_the_time( _x( 'j', 'daily archives date format', 'seeko' ) ) ) . '</a>';
...
class buddypress_Breadcrumb_Trail extends Breadcrumb_Trail {
	...
	public function do_trail_items() {
		...
		if ( ! empty( $bp->displayed_user->fullname ) ) { // looking at a user or self
			...
			$this->items[] = '<a href="' . esc_url( get_permalink( $bp->pages->groups->id ) ) . '" title="' . esc_attr( get_the_title( $bp->pages->groups->id ) ) . '">' . wp_kses_post( get_the_title( $bp->pages->groups->id ) ) . '</a>';

			$trail_end = __( 'Create a Group', 'buddypress' );

		} else if ( bp_is_create_blog() ) {
			$this->items[] = '<a href="' . esc_url( home_url( '/' ) . $bp->current_component ) . '" title="' . esc_attr( ucwords( $bp->current_component ) ) . '">' . wp_kses_post( ucwords( $bp->current_component ) ) . '</a>';
