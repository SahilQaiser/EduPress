...
class SQ_Addons_Manager {
		...
		if ( isset( $plugin_config['addon_type'] ) && $plugin_config['addon_type'] === 'child_theme' ) {
			...
			if ( $this->is_child_theme_installed() ) {
				...
				if ( $this->is_child_theme_active() ) {
					...
					$status['status_text'] = __( 'Active', 'seeko' );
					$status['action_text'] = __( 'Child theme installed and active', 'seeko' );
					$status['action']      = 'no_action';
				} else {
					$status['status']      = 'sq-inactive';
					$status['status_text'] = __( 'Inactive', 'seeko' );
					$status['action_text'] = __( 'Activate child theme', 'seeko' );
					$status['action']      = 'enable_child_theme';
				}
			} else {
				$status['status']      = 'sq-needs-install';
				$status['status_text'] = __( 'Not installed', 'seeko' );
				$status['action_text'] = __( 'Install child theme', 'seeko' );
				$status['action']      = 'install_theme';

				if ( ! current_user_can( 'install_themes' ) ) {
					$status['status']      = 'sq-not-installed sq-addons-disabled';
					$status['action_text'] = __( 'Permissions needed to install child themes. Contact site administrator.', 'seeko' );
			...
			if ( $this->is_plugin_installed( $plugin_slug ) ) {
				if ( $this->does_plugin_have_update( $plugin_slug ) ) {
					...
					$status['status_text'] = __( 'Needs update', 'seeko' );
					$status['action_text'] = __( 'Update plugin', 'seeko' );
					$status['action']      = 'update_plugin';
				} elseif ( $this->check_plugin_active( $plugin_slug ) ) {
					$status['status']      = 'sq-active';
					$status['status_text'] = __( 'Active', 'seeko' );
					$status['action_text'] = __( 'Deactivate plugin', 'seeko' );
					$status['action']      = 'disable_plugin';
				} else {
					$status['status']      = 'sq-inactive';
					$status['status_text'] = __( 'Inactive', 'seeko' );
					$status['action_text'] = __( 'Activate plugin', 'seeko' );
					$status['action']      = 'enable_plugin';
				}
			} else {
				$status['status']      = 'sq-not-installed';
				$status['status_text'] = __( 'Not Installed', 'seeko' );
				$status['action_text'] = __( 'Install plugin', 'seeko' );
				$status['action']      = 'install_plugin';

				if ( ! current_user_can( 'install_plugins' ) ) {
					$status['status']      = 'sq-not-installed sq-addons-disabled';
					$status['action_text'] = __( 'You don\'t have permission to install plugins. Contact site administrator.', 'seeko' );
					$status['action']      = 'contact_network_admin';
				}
	...
	public function check_plugin_active( $slug ) {
		return $this->tgmpa->is_active( $slug );
	}
