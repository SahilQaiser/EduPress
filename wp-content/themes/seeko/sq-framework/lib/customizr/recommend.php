			...
			class Kirki_Installer_Section extends WP_Customize_Section {
				...
				protected function render() {
					...
					foreach ( $plugins as $plugin ) {
						if ( 'SQ Theme Core' === $plugin['Name'] || 'Kirki Toolkit' === $plugin['Name'] || 'Kirki' === $plugin['Name'] ) {
							$installed = true;
						}
					}
				...
				protected function install_button() {
					...
					<p style="text-align:left;margin-top:0;">
						<?php esc_attr_e( 'Please install Theme Core plugin to take full advantage of this theme\'s customizer capabilities', 'seeko' ); ?>
					</p>
					<a class="install-now button-primary button" data-slug="sq-theme-core" href="<?php echo esc_url_raw( $this->get_plugin_install_url() ); ?>" aria-label="<?php esc_attr_e( 'Install Theme Core Plugin now', 'seeko' ); ?>" data-name="Theme Core">
						<?php esc_html_e( 'Install Now', 'seeko' ); ?>
					</a>
					<?php
				}
				...
				protected function activate_button() {
					...
					$url = wp_nonce_url ( $current_url . '?svq_action=activate_plugin&plugin=sq-theme-core', 'activate_plugin', 'svq_nonce');

					?>
					<p style="text-align:left;margin-top:0;">
						<?php esc_attr_e( 'You have installed Theme Core plugin. Activate it to start customizing your site.', 'seeko' ); ?>
					</p>
					<a class="activate-now button-primary button" data-slug="sq-theme-core"
					   href="<?php echo esc_url_raw( $url ); ?>"
					   aria-label="<?php esc_attr_e( 'Activate Theme Core Plugin now', 'seeko' ); ?>" data-name="Theme Core">
						<?php esc_html_e( 'Activate Now', 'seeko' ); ?>
					</a>
					<?php
				}
				...
				protected function dismiss_button() {
					...
					$show_confirm = true;
					?>
					<a class="kirki-installer-dismiss button-secondary button" data-slug="sq-theme-core" href="#" aria-label="<?php esc_attr_e( 'Don\'t show this again', 'seeko' ); ?>" data-name="<?php esc_attr_e( 'Dismiss', 'seeko' ); ?>">
						<?php esc_attr_e( 'Don\'t show this again', 'seeko' ); ?>
					</a>
					...
					jQuery( document ).ready( function() {
						jQuery( '.kirki-installer-dismiss' ).on( 'click', function( event ) {
							...
							<?php if ( $show_confirm ) : ?>
								if ( ! confirm( '<?php esc_attr_e( 'Are you sure? Dismissing this message will hide the installation recommendation and you will have to manually install and activate the plugin in the future.', 'seeko' ); ?>' ) ) {
									return;
								}
							<?php endif; ?>
				...
				private function get_plugin_install_url() {
					if ( ! $this->plugin_install_url ) {
						...
						$current_url= admin_url('customize.php');
						$this->plugin_install_url = wp_nonce_url ( $current_url . '?svq_action=activate_plugin&plugin=sq-theme-core', 'activate_plugin', 'svq_nonce');
					}
