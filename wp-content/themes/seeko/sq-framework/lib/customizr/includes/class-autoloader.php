 ...
 * @package Seeko\Customizer
 */
final class Seeko_Customizer_Autoloader {
	...
	public function autoload( $class_name ) {
		if ( class_exists( $class_name ) || 0 !== stripos( $class_name, 'Seeko_Customizer_' ) ) {
			return;
		}

		// Assumed file name.
		$file_name = strtolower( str_replace( [ 'Seeko_Customizer_', '_' ], [ '', '-' ], $class_name ) );
	...
	public function get_path( $file_name ) {
		...
		return [
			'dir'  => wp_normalize_path( SEEKO_CUSTOMIZER_PATH . $path['dir'] ),
...
new Seeko_Customizer_Autoloader();
