...
<?php if ( ! empty( $pmpro_invoice ) && ! empty( $pmpro_invoice->id ) ) { ?>
	...
	<div class="row my-3">
		...
		<div class="col-md-auto">
			<a class="btn btn-xs" href="javascript:window.print()">
	...
	<ul class="list-unstyled">
		...
		<li><strong><?php _e( 'Account', 'paid-memberships-pro' ); ?>
				:</strong> <?php echo esc_html( $current_user->display_name ); ?> (<?php echo esc_html( $current_user->user_email ); ?>)
		</li>
		<li><strong><?php _e( 'Membership Level', 'paid-memberships-pro' ); ?>
				:</strong> <?php echo esc_html( $current_user->membership_level->name ); ?></li>
		...
		<div class="pmpro_invoice_details row">
			...
			<?php if ( $pmpro_invoice->accountnumber ) { ?>
				<div class="pmpro_invoice-payment-method col-md-4">
					...
					<p><?php echo esc_html( $pmpro_invoice->cardtype ); ?><?php esc_html_e( 'ending in', 'paid-memberships-pro' ); ?><?php echo last4( $pmpro_invoice->accountnumber ) ?></p>
					<p><?php esc_html_e( 'Expiration', 'paid-memberships-pro' ); ?>
	...
	<ul class="list-unstyled">
		...
		<li><strong><?php _e( 'Membership Level', 'paid-memberships-pro' ); ?>
				:</strong> <?php if ( ! empty( $current_user->membership_level ) ) {
				echo esc_html( $current_user->membership_level->name );
			} else {
				esc_html_e( "Pending", 'paid-memberships-pro' );
			} ?></li>
	</ul>
	<?php
}
