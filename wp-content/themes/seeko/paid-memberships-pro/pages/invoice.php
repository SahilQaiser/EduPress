...
if ( $pmpro_invoice ) {
	...
	<div class="row my-3">
		...
		<div class="col-md-auto">
			<a class="pmpro_a-print btn btn-xs" href="javascript:window.print()">
				<i class="icon icon-print"></i><span><?php _e( 'Print', 'paid-memberships-pro' ); ?></span>
			</a>
		</div>
	</div>
	...
	<ul class="list-unstyled">
		...
		<li><strong><?php _e( 'Account', 'paid-memberships-pro' ); ?>
				:</strong> <?php echo esc_html( $pmpro_invoice->user->display_name ); ?>
			(<?php echo esc_html( $pmpro_invoice->user->user_email ); ?>)
		</li>
		<li><strong><?php esc_html_e( 'Membership Level', 'paid-memberships-pro' ); ?>
				:</strong> <?php echo esc_html( $pmpro_invoice->membership_level->name ); ?></li>
		<?php if ( $current_user->membership_level->enddate ) { ?>
			<li><strong><?php esc_html_e( 'Membership Expires', 'paid-memberships-pro' ); ?>
					:</strong> <?php echo date_i18n( get_option( 'date_format' ), $current_user->membership_level->enddate ) ?>
			</li>
		<?php } ?>
		<?php if ( $pmpro_invoice->getDiscountCode() ) { ?>
			<li><strong><?php _e( 'Discount Code', 'paid-memberships-pro' ); ?>
	...
	<div class="container-fluid bg-light p-4 rounded">
		<div class="pmpro_invoice_details row">
			<?php if ( ! empty( $pmpro_invoice->billing->name ) ) { ?>
				<div class="pmpro_invoice-billing-address col-md-4">
			...
			<?php if ( $pmpro_invoice->accountnumber ) { ?>
				<div class="pmpro_invoice-payment-method col-md-4">
					<strong><?php esc_html_e( 'Payment Method', 'paid-memberships-pro' ); ?></strong>
					<p><?php echo esc_html( $pmpro_invoice->cardtype ); ?><?php _e( 'ending in', 'paid-memberships-pro' ); ?><?php echo last4( $pmpro_invoice->accountnumber ) ?></p>
					<p><?php _e( 'Expiration', 'paid-memberships-pro' ); ?>
						: <?php echo esc_html( $pmpro_invoice->expirationmonth ); ?>/<?php echo esc_html( $pmpro_invoice->expirationyear ); ?></p>
				</div> <!-- end pmpro_invoice-payment-method -->
			<?php } elseif ( $pmpro_invoice->payment_type ) { ?>
				<?php echo esc_html( $pmpro_invoice->payment_type ); ?>
			<?php } ?>

			<div class="pmpro_invoice-total col-md-4">
				...
				<p><?php if ( $pmpro_invoice->total != '0.00' ) { ?>
						<?php if ( ! empty( $pmpro_invoice->tax ) ) { ?>
							...
							<br/>
							<?php _e( 'Tax', 'paid-memberships-pro' ); ?>: <?php echo pmpro_formatPrice( $pmpro_invoice->tax ); ?>
							<br/>
							<?php if ( ! empty( $pmpro_invoice->couponamount ) ) { ?>
								<?php _e( 'Coupon', 'paid-memberships-pro' ); ?>: (<?php echo pmpro_formatPrice( $pmpro_invoice->couponamount ); ?>)
								<br/>
							<?php } ?>
							<strong><?php _e( 'Total', 'paid-memberships-pro' ); ?>
	...
	if ( $invoices ) {
		...
		<table id="pmpro_invoices_table" class="pmpro_invoice" width="100%" cellpadding="0" cellspacing="0" border="0">
			...
			foreach ( $invoices as $invoice ) {
				...
				<tr>
					<td>
						<a href="<?php echo pmpro_url( "invoice", "?invoice=" . $invoice->code ) ?>"><?php echo date_i18n( get_option( "date_format" ), $invoice->timestamp ) ?></a>
					</td>
					<td>
						<a href="<?php echo pmpro_url( "invoice", "?invoice=" . $invoice->code ) ?>"><?php echo esc_html( $invoice->code ); ?></a>
					</td>
