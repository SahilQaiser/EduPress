	...
	<form id="pmpro_form" class="pmpro_form" action="<?php if ( ! empty( $_REQUEST['review'] ) ) {
		...
		<?php if ( $pmpro_msg ) { ?>
			<div id="pmpro_message"
			     class="alert alert-info pmpro_message <?php echo esc_attr( $pmpro_msgt ); ?>"><?php echo wp_kses_post( $pmpro_msg ); ?></div>
		<?php } else { ?>
			<div id="pmpro_message" class="alert alert-info pmpro_message" style="display: none;"></div>
		<?php } ?>

		<?php if ( $pmpro_review ) { ?>
			<p><?php echo wp_kses_post( __( 'Almost done. Review the membership information and pricing below then <strong>click the "Complete Payment" button</strong> to finish your order.', 'paid-memberships-pro' ) ); ?></p>
		<?php } ?>
		...
		if ( $include_pricing_fields ) {
			...
			<div id="pmpro_pricing_fields" class="pmpro_checkout">
				<h3 class="h4">
				<span
					class="pmpro_checkout-h3-name"><?php esc_html_e( 'Membership Level', 'paid-memberships-pro' ); ?></span>
					<?php if ( count( $pmpro_levels ) > 1 ) { ?>
						<span class="pmpro_checkout-h3-msg">
					<a class="btn btn-xs mb-0 ml-3"
					   href="<?php echo pmpro_url( "levels" ); ?>"><?php esc_html_e( 'change', 'paid-memberships-pro' ); ?></a>
				</span>
					<?php } ?>
				</h3>
				<div class="pmpro_checkout-fields">
					<p>
						<?php echo wp_kses_post( sprintf( __( 'You have selected the <strong>%s</strong> membership level.', 'paid-memberships-pro' ), $pmpro_level->name ) ); ?>
					</p>
					...
					<?php if ( $pmpro_show_discount_code ) { ?>
						...
						<?php } elseif ( $pmpro_review && $discount_code ) { ?>
							<p><strong><?php _e( 'Discount Code', 'paid-memberships-pro' ); ?>
									:</strong> <?php echo esc_html( $discount_code ); ?></p>
						<?php } ?>
					<?php } ?>
						...
						<div id="other_discount_code_tr" style="display: none;">
							...
							<div class="row">
								<div class="input-group col-md-6 mb-3">
									<input class="form-control" id="other_discount_code" name="other_discount_code"
									       type="text"
									       class="input <?php echo pmpro_getClassForField( "other_discount_code" ); ?>"
									       size="20" value="<?php echo esc_attr( $discount_code ); ?>"/>
									<div class="input-group-append">
										<input class="btn" type="button" name="other_discount_code_button"
		...
		<?php if ( ! $skip_account_fields && ! $pmpro_review ) { ?>
			...
			<div id="pmpro_user_fields" class="pmpro_checkout">
				<h3 class="h4">
					<span
						class="pmpro_checkout-h3-name"><?php _e( 'Account Information', 'paid-memberships-pro' ); ?></span>
					<span
						class="pmpro_checkout-h3-msg"><?php _e( 'Already have an account?', 'paid-memberships-pro' ); ?>
						<a href="<?php echo esc_url( wp_login_url( pmpro_url( "checkout", "?level=" . $pmpro_level->id ) ) ); ?>"><?php esc_html_e( 'Log in here', 'paid-memberships-pro' ); ?></a></span>
				</h3>
				<div class="pmpro_checkout-fields row">
					<div class="pmpro_checkout-field pmpro_checkout-field-username form-group col-md-4">
					...
					<div class="pmpro_checkout-field pmpro_checkout-field-password form-group col-md-4">
					...
					if ( $pmpro_checkout_confirm_password ) { ?>
						<div class="pmpro_checkout-field pmpro_checkout-field-password2 form-group col-md-4">
					...
					<div class="pmpro_checkout-field pmpro_checkout-field-bemail form-group col-md-6">
						<label for="bemail"><?php _e( 'E-mail Address', 'paid-memberships-pro' ); ?></label>
						<input id="bemail" name="bemail"
						       type="<?php echo esc_html( $pmpro_email_field_type ? 'email' : 'text' ); ?>"
					...
					if ( $pmpro_checkout_confirm_email ) { ?>
						<div class="pmpro_checkout-field pmpro_checkout-field-bconfirmemail form-group col-md-6">
							<label
					...
					<div class="pmpro_hidden">
						...
						<input id="fullname" name="fullname" type="text"
						       class="input <?php echo pmpro_getClassForField( "fullname" ); ?>" size="30" value=""/>
		...
		<?php } elseif ( $current_user->ID && ! $pmpro_review ) { ?>
			<div id="pmpro_account_loggedin" class="pmpro_message pmpro_alert alert alert-warning">
				<?php echo wp_kses_post( sprintf( __( 'You are logged in as <strong>%s</strong>. If you would like to use a different account for this membership, <a href="%s">log out now</a>.', 'paid-memberships-pro' ), $current_user->user_login, wp_logout_url( $_SERVER['REQUEST_URI'] ) ) ); ?>
		...
		<?php if ( pmpro_getGateway() == "paypal" && empty( $pmpro_review ) && true == apply_filters( 'pmpro_include_payment_option_for_paypal', true ) ) { ?>
			<div id="pmpro_payment_method" class="pmpro_checkout"
				...
				<h4>
					<span
						class="pmpro_checkout-h3-name"><?php _e( 'Choose your Payment Method', 'paid-memberships-pro' ); ?></span>
				</h4>
				<div class="pmpro_checkout-fields">
		...
		if ( $pmpro_include_billing_address_fields ) { ?>
			...
			<div id="pmpro_billing_address_fields" class="pmpro_checkout"
				...
				<div class="row  justify-content-md-center">
					<div class="col-lg-12">
						<h4><span class="pmpro_checkout-h3-name"><?php _e( 'Billing Address', 'paid-memberships-pro' ); ?></span></h4>
						<div class="pmpro_checkout-fields row">
							<div class="pmpro_checkout-field pmpro_checkout-field-bfirstname form-group col-md-6">
							...
							<div class="pmpro_checkout-field pmpro_checkout-field-blastname form-group col-md-6">
							...
							<div class="pmpro_checkout-field pmpro_checkout-field-baddress1 form-group col-md-6">
							...
							<div class="pmpro_checkout-field pmpro_checkout-field-baddress2 form-group col-md-6">
							...
							if ( $longform_address ) { ?>
								<div class="pmpro_checkout-field pmpro_checkout-field-bcity form-group col-md-4">
								...
								<div class="pmpro_checkout-field pmpro_checkout-field-bstate form-group col-md-4">
								...
								<div class="pmpro_checkout-field pmpro_checkout-field-bzipcode form-group col-md-4">
							...
							<?php } else { ?>
								<div
									class="pmpro_checkout-field pmpro_checkout-field-bcity_state_zip form-group col-md-12">
									<label
										for="bcity_state_zip"><?php _e( 'City, State Zip', 'paid-memberships-pro' ); ?></label>
									<div class="row">
										<div class="col">
											<input id="bcity" name="bcity" type="text"
												...
												<select name="bstate"
													...
													foreach ( $pmpro_states as $ab => $st ) { ?>
														<option value="<?php echo esc_attr( $ab ); ?>"
														        <?php if ( $ab == $bstate ) { ?>selected="selected"<?php } ?>><?php echo esc_html( $st ); ?></option>
													<?php } ?>
												</select>
											<?php } elseif ( $state_dropdowns == "abbreviations" ) {
												...
												<select name="bstate"
													...
													foreach ( $pmpro_states_abbreviations as $ab ) {
														?>
														<option value="<?php echo esc_attr( $ab ); ?>"
														        <?php if ( $ab == $bstate ) { ?>selected="selected"<?php } ?>><?php echo esc_html( $ab ); ?></option>
													<?php } ?>
												</select>
											<?php } else { ?>
												<input id="bstate" name="bstate" type="text"
							...
							if ( $show_country ) { ?>
								<div class="pmpro_checkout-field pmpro_checkout-field-bcountry form-group col-md-6">
									...
									<select name="bcountry" id="bcountry"
										...
										foreach ( $pmpro_countries as $abbr => $country ) { ?>
											<option value="<?php echo esc_attr( $abbr ); ?>"
											        <?php if ( $abbr == $bcountry ) { ?>selected="selected"<?php } ?>><?php echo esc_html( $country ); ?></option>
										<?php } ?>
									</select>
							...
							<?php } ?>
							<div class="pmpro_checkout-field pmpro_checkout-field-bphone form-group col-md-6">
							...
							<?php if ( $skip_account_fields ) { ?>
								...
								if ( $pmpro_checkout_confirm_email ) { ?>
									<div
										class="pmpro_checkout-field pmpro_checkout-field-bconfirmemail form-group col-md-6">
										<label
		...
		if ( $pmpro_include_payment_information_fields ) { ?>
			...
			<div id="pmpro_payment_information_fields" class="pmpro_checkout"
				...
				<div class="row justify-content-md-center">
					<div class="col-lg-12">
						<hr class=""/>
						<h4><span class="pmpro_checkout-h3-name"><?php esc_html_e( 'Payment Information', 'paid-memberships-pro' ); ?></span>
							<span class="pmpro_checkout-h3-msg"><?php printf( esc_html__( 'We Accept %s', 'paid-memberships-pro' ), $pmpro_accepted_credit_cards_string ); ?></span>
						</h4>
						...
						<?php } ?>
							<div class="pmpro_checkout-fields row">
								...
								if ( $pmpro_include_cardtype_field ) { ?>
									<div class="pmpro_checkout-field pmpro_payment-card-type form-group col-md-6">
										<label
											for="CardType"><?php _e( 'Card Type', 'paid-memberships-pro' ); ?></label>
								...
								<?php }
								$class = $pmpro_include_cardtype_field ? 'col-md-6' : 'col-md-12';
								?>
								<div class="pmpro_checkout-field pmpro_payment-account-number form-group <?php echo esc_html( $class ); ?>">
									<label for="AccountNumber"><?php _e( 'Card Number', 'paid-memberships-pro' ); ?></label>
									<input id="AccountNumber" name="AccountNumber"
									       class="input <?php echo pmpro_getClassForField( "AccountNumber" ); ?>"
									       type="text"
									       size="30" value="<?php echo esc_attr( $AccountNumber ); ?>"
									       data-encrypted-name="number" autocomplete="off"/>
								</div>
								<div class="pmpro_checkout-field pmpro_payment-expiration form-group col-md-12">
											...
											<select id="ExpirationYear" name="ExpirationYear"
												...
												for ( $i = date_i18n( "Y" ); $i < intval( date_i18n( "Y" ) ) + 10; $i ++ ) {
													...
													<option value="<?php echo esc_attr( $i ) ?>"
													        <?php if ( $ExpirationYear == $i ) { ?>selected="selected"<?php } ?>><?php echo esc_html( $i ) ?></option>
													<?php
												}
												?>
											</select>
								...
								if ( $pmpro_show_cvv ) { ?>
									<div class="pmpro_checkout-field pmpro_payment-cvv form-group col-md-6">
										<label
											for="CVV"><?php _e( 'Security Code (CVC)', 'paid-memberships-pro' ); ?></label>
								...
								<?php } ?>
								<?php if ( $pmpro_show_discount_code ) { ?>
									<div class="pmpro_checkout-field pmpro_payment-discount-code form-group col-md-12">
										<label
										...
										<div class="row">
											<div class="input-group col-md-6">
												...
												<div class="input-group-append">
													<input class="btn" type="button" id="discount_code_button"
													       name="discount_code_button"
													       value="<?php _e( 'Apply', 'paid-memberships-pro' ); ?>"/>
												</div>
											</div>
										</div>

										<p id="discount_code_message" class="alert alert-info pmpro_message mt-4"
		...
		<?php if ( $tospage && ! $pmpro_review ) { ?>
			<div id="pmpro_tos_fields" class="pmpro_checkout">
				<hr/>
				<h4>
					<span class="pmpro_checkout-h3-name mr-3"><?php echo esc_html( $tospage->post_title ); ?></span>
				</h4>
				<div class="pmpro_checkout-fields">
					...
					<input type="checkbox" name="tos" value="1" id="tos"/> <label
						class="pmpro_label-inline pmpro_clickable"
		...
		<div class="pmpro_submit pb-4">
          ...
          <input type="submit" class="pmpro_btn pmpro_btn-submit-checkout btn btn-primary"
                 value="<?php _e( 'Complete Payment', 'paid-memberships-pro' ); ?> &raquo;"/>
					...
					<?php } else { ?>
						...
						if ( $pmpro_checkout_default_submit_button ) {
							?>
							<span id="pmpro_submit_span" class="d-block mt-4">
        ...
        <?php
        $processing_message = apply_filters( "pmpro_processing_message", esc_html__( "Processing...", 'paid-memberships-pro' ) );
...
<script>
	window.addEventListener('DOMContentLoaded', function () {
		...
		jQuery('.pmpro_required').after('<span class="pmpro_asterisk"> <abbr title="Required Field">*</abbr></span>')
			.each(function () {
				jQuery(this).next('.pmpro_asterisk').andSelf().wrapAll("<div class='form-asterisk' />");
			});
		...
		jQuery('#other_discount_code').on('keydown', function (e) {
			if (e.keyCode == 13) {
				e.preventDefault();
				jQuery('#other_discount_code_button').trigger('click');
			}
		});
	});
</script>
